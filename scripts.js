...
function showContent(tabName) {
    ...
    if (activeContent) {
        ...
        console.error(`L'élément avec l'ID ${tabName}-content n'existe pas.`);
    }

    // Activer le bouton de l'onglet sélectionné
    const activeButton = document.querySelector(`button[onclick="showContent('${tabName}')"]`);
    if (activeButton) {
        activeButton.classList.add('active');
    } else {
        console.error(`Le bouton pour l'onglet ${tabName} n'existe pas.`);
    }
}
